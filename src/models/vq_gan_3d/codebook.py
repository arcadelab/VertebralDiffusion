...
class Codebook(nn.Module):
    ...
    def forward(self, z):
        ...
        vq_loss = 0.25 * F.mse_loss(embeddings, z.detach())

        # EMA codebook update
        if self.training and self.update_codebook:
            # print(f"DEBUG: updating codebook")
            n_total = encode_onehot.sum(dim=0) # count each node
            ...
            n = self.N.sum()
            weights = (self.N + 1e-4) / (n + self.n_codes * 1e-4) * n
        ...
        perplexity = torch.exp(-torch.sum(avg_probs *
                               torch.log(avg_probs + 1e-4)))

        return dict(embeddings=embeddings_st, encodings=encoding_indices,
                    commitment_loss=commitment_loss + vq_loss, perplexity=perplexity)
