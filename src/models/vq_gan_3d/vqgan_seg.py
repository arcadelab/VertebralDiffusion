...
from vq_gan_3d.model.dice_loss import MulticlassDiceLoss
...
def get_seg_weights():
    seg_weights = [1 for _ in range(40)]
    seg_weights[0] = 1
    seg_weights[1] = 5
    seg_weights[-1] = 5
    # seg_weights=[1.56790290e-03, 4.04335185e-02, 3.04096892e-02, 4.29161462e-01, 
    #             4.67524654e-03, 1.41291525e-02, 1.60220663e-01, 2.31234996e+00,
    #     1.77246809e+00, 7.05778682e-03, 7.77397518e-03, 3.17366394e-02,
    #     2.10592067e-01, 3.60218664e-01, 8.18148685e-01, 7.71098104e-03,
    #     1.31635976e-01, 6.44075744e-03, 4.86911644e-02, 6.42399092e-01,
    #     8.59450659e+00, 1.29128158e+01, 1.72501250e-02, 6.80041160e-02,
    #     5.44972797e-01, 2.61273046e+00, 9.51419821e-01, 1.33649002e+00,
    #     5.72700106e-01, 1.81554579e+00, 4.02333303e-01, 1.80864161e-01,
    #     1.05135605e+00, 5.66922395e-01, 3.00802433e-01, 5.45154073e-03,
    #     8.20088692e-03, 1.71391036e-02, 2.67306753e-03]
    return seg_weights
...
class VQGAN_SEG(pl.LightningModule):
    def __init__(self, cfg):
        ...
        # self.dice_loss = DiceLoss(include_background=False, softmax=False, squared_pred=True)
        # self.dice_loss = nn.L1Loss()
    ...
    def forward(self, x, optimizer_idx=None, log_image=False):
        ...
        self.unique_indices.update(torch.unique(vq_output["encodings"].detach()).tolist())
        x_recon = self.decoder(self.post_vq_conv(vq_output["embeddings"]))
        assert (
            x_recon.shape == x.shape
        ), f"SHAPE MISMATCH, x_recon.shape: {x_recon.shape}, x.shape: {x.shape}"
        # recon_loss = F.l1_loss(x_recon, x) * self.l1_weight
        recon_loss = self.dice_loss(x_recon, x) * self.l1_weight
        # lc_mask = torch.where(torch.logical_and(x.detach() > 1.5, x.detach() < 2.75), torch.ones_like(x), torch.zeros_like(x).to(x.device)).float()
        # hc_mask = torch.where(x.detach() > 2.75, torch.ones_like(x), torch.zeros_like(x).to(x.device)).float()
        # # Low contrast information loss
        # recon_loss += F.l1_loss(x_recon*lc_mask, x*lc_mask) * self.lc_weight
        # # High contrast information loss
        # recon_loss += F.l1_loss(x_recon*hc_mask, x*hc_mask) * self.hc_weight
        # print(f"L1 weight: {self.l1_weight}, L1 loss: {recon_loss}")

        # QA: Why select only one frame?
        ...
        if optimizer_idx == 0:
            ...
            self.log("train/codebook_util_rate", len(self.unique_indices) / self.n_codes)
    ...
    def validation_step(self, batch, batch_idx):
        x = batch["seg"].as_tensor()  # TODO: batch['stft']
    ...
    def seg_to_onehot(self, x):
        # print(f"seg_to_onehot: x.shape: {x.shape}")
        num_classes = self.cfg.dataset.seg_channels
        x = torch.argmax(x, dim=1)
        # print(f"argmax: x.shape: {x.shape}")
        if len(x.shape) == 3:
            N, H, W = x.shape
            x = (
            ...
            x = (
                ...
                .contiguous()
                .permute(0, 4, 1, 2, 3)
                .view(N, num_classes, H, W, D)
            )
        # print(f"after seg_to_onehot: x.shape: {x.shape}")
        return x
    ...
    def log_images(self, batch, **kwargs):
        ...
        log["inputs"] = torch.argmax(frames, dim=1, keepdim=True).float()
        log["reconstructions"] = torch.argmax(frames_rec, dim=1, keepdim=True).float()
        # log['mean_org'] = batch['mean_org']
        # log['std_org'] = batch['std_org']
        return log

    def log_videos(self, batch, **kwargs):
        to_onehot = mt.AsDiscrete(argmax=True, to_onehot=self.cfg.dataset.seg_channels)
        log = dict()
        x = batch["seg"].as_tensor()
        _, _, x, x_rec = self(x, log_image=True)
        log["inputs"] = torch.argmax(x, dim=1, keepdim=True).float()
        log["reconstructions"] = torch.argmax(x_rec, dim=1, keepdim=True).float()
        # log['mean_org'] = batch['mean_org']
        # log['std_org'] = batch['std_org']
        return log

    def encode_raw_lg(
        self, img_raw, spatial_size, compression_rate, include_embeddings=False, overlap=0.25
    ):
        # expect x_raw to have shape (1,C,H,W,D)
        with torch.no_grad():
            x_raw = x_raw.to("cpu")
            sliding_window_infere = SlidingWindowInferer(
            ...
            def wrapper(x):
                embeddings, _ = self.encode(x.to(self.device), include_embeddings=True)
                return embeddings.to("cpu")

            embeddings = sliding_window_infere(img_raw, wrapper)
    ...
    def decode_raw_lg(
        self, latent, spatial_size, compression_rate, quantize=False, overlap=0.25, progress=True
    ):
        # expect latent to have shape (1,H,W,D) or (C,H,W,D) if quantize
        ...
        with torch.no_grad():
            ...
            sliding_window_infere = SlidingWindowInferer(
                roi_size=[s // compression_rate for s in spatial_size],
                mode="gaussian",
                overlap=overlap,
                # overlap=0,
                progress=progress,
            ...
            def wrapper(x):
                # if quantize:
                #     vq_output = self.codebook(x.to(self.device))
                #     x = vq_output["encodings"]
                ...
                if not quantize:
                    ...
                    # print(f"not quantize x.shape: {x.shape}")
                    x_recon = self.decode(
                        x.round().long().to(self.device), quantize=quantize
                    )  # now is (4,96,96,96)
                else:
                    # print(f"x.shape: {x.shape}")
                    x_recon = self.decode(x.to(self.device), quantize=quantize) # now is (4,96,96,96)
                # print(f"seg decode raw lg wrapper x_recon.shape: {x_recon.shape}")
                # exit(-1)
                return x_recon#.to('cpu')
            # print(f"before seg sliding")
            # x_recon = sliding_window_infere(latent.float().to(self.device), wrapper) # sliding window does not work with long
            x_recon = sliding_window_infere(latent.float(), wrapper) # sliding window does not work with long
            # print(f"after seg sliding, x_recon.shape: {x_recon.shape}")
            x_recon = torch.argmax(x_recon, dim=1, keepdim=True)
            # x_recon = x_recon.round().long()
        # exit(-1)
        return x_recon.to('cpu')

    def reconstruct_lg(self, x_raw, spatial_size, overlap=0.25):
        # expect x_raw to have shape (1,C,H,W,D)
        # assuming x_raw on cpu and self on device
        # x_raw should be larger than spatial_size
        with torch.no_grad():
            # print(f"x_raw.shape: {x_raw.shape}")
            x_raw = x_raw.to("cpu")
            sliding_window_infere = SlidingWindowInferer(
            ...
            def wrapper(x):
                x = self.encode(x.to(self.device), include_embeddings=False)
                x_recon = self.decode(x, quantize=False) # now is (4,96,96,96)
                return x_recon#.to('cpu')
            x_recon = sliding_window_infere(x_raw, wrapper)
            return x_recon
...
class SamePadConv3d(nn.Module):
    def __init__(
        self, in_channels, out_channels, kernel_size, stride=1, bias=True, padding_type="replicate"
    ):
        super().__init__()
